...
class checkpoint():
    ...
    def save_results(self, filename, save_list, scale):
        ...
        for v, p in zip(save_list, postfix):
            ...
            # ✅ Clean & correct saving
            v = torch.clamp(v, 0, 1)
            normalized = v[0] * 255.0
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()

            imageio.imsave('{}/{}.png'.format(folder, filename), ndarr)
...
def calc_psnr(sr, hr, scale, rgb_range, benchmark=False):
    ...
    if diff.size(1) > 1:
        ...
        convert[0, 0, 0, 0] = 65.738 / 256
        convert[0, 1, 0, 0] = 129.057 / 256
        convert[0, 2, 0, 0] = 25.064 / 256
